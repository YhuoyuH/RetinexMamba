...
class IG_MSA(nn.Module):
    """
    实现交错组多头自注意力（IG-MSA）模块。

    属性:
        dim (int): 输入特征的维度。即为shape中元素的个数
        dim_head (int): 多头注意力机制中每个头的维度。
        heads (int): 多头注意力机制中头的数量。
        to_q (nn.Linear): 查询的线性变换。
        to_k (nn.Linear): 键的线性变换。
        to_v (nn.Linear): 值的线性变换。
        rescale (nn.Parameter): 应用于注意力得分的重标定因子。
        proj (nn.Linear): 输出特征维度的线性投影。
        pos_emb (nn.Sequential): 位置嵌入层。
    """

    def __init__(self, dim, dim_head=64, heads=8):
        super().__init__()
        self.num_heads = heads  # 头的数量
        self.dim_head = dim_head  # 每个头的维度
        self.dim = dim  # 输入特征的维度
        self.to_q = nn.Linear(dim, dim_head * heads, bias=False)  # 用于生成查询向量的线性变换
        self.to_k = nn.Linear(dim, dim_head * heads, bias=False)  # 用于生成键向量的线性变换
        self.to_v = nn.Linear(dim, dim_head * heads, bias=False)  # 用于生成值向量的线性变换
        self.rescale = nn.Parameter(torch.ones(heads, 1, 1))  # 用于重标定注意力得分的参数
        self.proj = nn.Linear(dim_head * heads, dim, bias=True)  # 输出的线性投影
        self.pos_emb = nn.Sequential(
            nn.Conv2d(dim, dim, 3, 1, 1, bias=False, groups=dim),  # 位置卷积层1
            GELU(),  # GELU激活函数
            nn.Conv2d(dim, dim, 3, 1, 1, bias=False, groups=dim),  # 位置卷积层2
        )
    ...
    def forward(self, x_in, illu_fea_trans):
        ...
        参数:
        x_in: 输入特征，形状为 [b, h, w, c]
        illu_fea_trans: 光照特征变换，原形状为 [b, c, h, w]，需要重排为 [b, h, w, c],因此后续在调用的时候就转成了[b, h, w, c]

        返回:
        输出特征，形状为 [b, h, w, c]
        """

        b, h, w, c = x_in.shape  # 提取输入的批量大小、高度、宽度和通道数
        x = x_in.reshape(b, h * w, c)  # 重塑输入以适应线性变换


        q_inp = self.to_q(x)  # 计算查询
        k_inp = self.to_k(x)  # 计算键
        v_inp = self.to_v(x)  # 计算值

        illu_attn = illu_fea_trans  # 从参数获取光照特征变换

        # 重排查询、键、值和光照特征以适应多头注意力格式
        q, k, v, illu_attn = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=self.num_heads),
                                 (q_inp, k_inp, v_inp, illu_attn.flatten(1, 2)))#利用lambda表达式
        ...
        attn = (k @ q.transpose(-2, -1))  # 计算注意力得分
        attn = attn * self.rescale  # 应用重标定因子
        attn = attn.softmax(dim=-1)  # 对注意力得分进行softmax处理
        x = attn @ v  # 通过注意力得分更新值
        x = x.permute(0, 3, 1, 2)  # 转置回原始维度(b,c,w,h)
        x = x.reshape(b, h * w, self.num_heads * self.dim_head)  # 重塑为原始的高度和宽度
        out_c = self.proj(x).view(b, h, w, c)  # 应用输出投影并调整形状
        out_p = self.pos_emb(v_inp.reshape(b, h, w, c).permute(
            0, 3, 1, 2)).permute(0, 2, 3, 1)  # 应用位置嵌入并调整形状
        out = out_c + out_p  # 将位置嵌入结果和主要输出相加
...
class IGAB(nn.Module):
    ...
    def __init__(self, dim, dim_head=64, heads=8, num_blocks=2):
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                IG_MSA(dim=dim, dim_head=dim_head, heads=heads),  # 自定义的多头自注意力模块
    ...
    def forward(self, x, illu_fea):
        ...
        for (attn, ff) in self.blocks:
            # 应用注意力模块并进行残差连接
            x = attn(x, illu_fea_trans=illu_fea.permute(0, 2, 3, 1)) + x  #当我创建了一个类之后,如果继承nn并且自己定义了一个forward,那么nn会把hook挂到对象中,直接用对象(forward的参数)就能调用forward函数
            #这里illu_fea.permute(0, 2, 3, 1)的原因是为了让illu_fea图片的维度顺序与输入x的对上,二者都变成(b,h,w,c)
            # 应用前馈网络并进行残差连接
            x = ff(x) + x
        out = x.permute(0, 3, 1, 2)  # 将维度调整回原始形状[b, c, h, w]
        return out
    ...
    def __init__(self, in_dim=3, out_dim=3, dim=31, level=2, num_blocks=[2, 4, 4]):
        ...
        for i in range(level):#每一级由于对应的特征维数不同，所以IGAB中的块数也不同，也要采用不同的头数
            self.encoder_layers.append(nn.ModuleList([
                IGAB(dim=dim_level, num_blocks=num_blocks[i], dim_head=dim, heads=dim_level // dim),#确定每一次调用IGAB时所需要的块数
        ...
        # 瓶颈层，处理最深层的特征，使用IGAB模块进行自注意力计算。
        self.bottleneck = IGAB(dim=dim_level, dim_head=dim, heads=dim_level // dim, num_blocks=num_blocks[-1])#这一个IGAB对应4C特征维度的
...
class RetinexFormer_Single_Stage(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, level=2, num_blocks=[1, 1, 1]):
        ...
        self.denoiser = Denoiser(in_dim=in_channels, out_dim=out_channels, dim=n_feat, level=level, num_blocks=num_blocks)  # 编解码获取最终结果
...
class RetinexFormer(nn.Module):
    """
    多阶段 Retinex 图像处理网络，每个阶段都通过 RetinexFormer_Single_Stage 来实现，
    进行图像的照明估计和增强。
    """
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1, 1, 1]):
        ...
        参数:
            ...
            stage (int): 网络包含的阶段数，每个阶段都使用一个 RetinexFormer_Single_Stage 模块。
            num_blocks (list): 每个阶段的块数，指定每个单阶段中的块数量。
        """
        super(RetinexFormer, self).__init__()
        self.stage = stage  # 网络的阶段数

        # 创建多个 RetinexFormer_Single_Stage 实例，每个实例都作为网络的一个阶段
