...
def read_img_seq(path, require_mod_crop=False, scale=1):
    ...
    参数:
        ...
        require_mod_crop (bool): 是否对每张图像进行模数裁剪。默认为False。
        scale (int): 对于mod_crop的缩放因子。默认为1。

    返回:
        Tensor: 形状为(t, c, h, w)的张量，RGB格式，值范围在[0, 1]之间。
    """
    # 判断路径是列表还是字符串，并 accordingly 获取图像路径
    ...
    # 如果需要进行模数裁剪，则对图像列表进行裁剪
    if require_mod_crop:
        imgs = [mod_crop(img, scale) for img in imgs]
    
    # 将图像从BGR转换为RGB，然后转换为张量
...
def generate_frame_indices(crt_idx, max_frame_num, num_frames, padding='reflection'):
    """
    从图像序列中生成用于读取`num_frames`帧的索引列表。

    参数:
        crt_idx (int): 当前中心索引。
        max_frame_num (int): 图像序列的最大帧数（从1开始）。
        num_frames (int): 要读取的帧数。
        padding (str): 填充模式，包括'replicate'、'reflection'、'reflection_circle'、'circle'。

    返回:
        list[int]: 索引列表。
    """
    # 检查num_frames是否为奇数
    assert num_frames % 2 == 1, 'num_frames应为奇数。'
    
    # 检查padding是否为有效值
    assert padding in ('replicate', 'reflection', 'reflection_circle', 'circle'), f'错误的填充模式: {padding}.'
    ...
    for i in range(crt_idx - num_pad, crt_idx + num_pad + 1):
        ...
        elif i > max_frame_num:  # 对于大于最大帧数的索引进行处理
            if padding == 'replicate':
